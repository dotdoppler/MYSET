...
###2016/4/18
####jQuery 解析JSON 把数据添加到页面中

###2016/4/19
####检查是否已有当前的书，没有就插入一条记录，有就返回并提示已经有了。
####拿到书之后加入侧边栏，下一步是动态地加载所有书的信息。



###2016/4/22
####发现几个个严重的问题，session里只有一个用户。不登陆也可以直接进入主页。
####加载图书信息完成。两个大bug未解决。

###2016/4/23
####原来是同一个浏览器中session被覆盖了，方向完全搞错了，哎，就当复习session和cookie的知识了吧....


###2016/4/24
####弄了个拦截器，防止未登陆时进入主页，注意重定向或者请求转发的时候不要进入死循环。
